# -*- coding: utf-8 -*-
"""Estimator for the recent-image check of 50 existing reference points.
Input : data/analyses/visual_check_50pts.csv  (per point: stratum, image date, agreement at the pixel scale of the classifier's
...
Output: results/analyses/visual_check_estimates.json"""
import csv,collections,math,json,os
HERE=os.path.dirname(os.path.abspath(__file__)); REPO=os.path.dirname(os.path.dirname(HERE))
rows=list(csv.DictReader(open(os.path.join(REPO,'data','analyses','visual_check_50pts.csv'),encoding='utf8')))
...
os.makedirs(os.path.join(REPO,'results','analyses'),exist_ok=True)
json.dump(o,open(os.path.join(REPO,'results','analyses','visual_check_estimates.json'),'w'),indent=1)
